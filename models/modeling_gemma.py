    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_rope_per_head:
            # Ensure inv_freq is initialized (Gemma lazily creates it)
            if self.rotary_emb.inv_freq is None:
                self.rotary_emb.inv_freq = 1.0 / (
                    self.rotary_emb.base ** (torch.arange(0, self.rotary_emb.dim, 2, dtype=torch.int64, device=value_states.device).float() / self.rotary_emb.dim)
                )
...
class GemmaSdpaAttention(GemmaAttention):
    ...
    def forward(
        ...
        if self.applied_module == 'attention':
            attn_output.clone()
            for i,(A,v) in enumerate(zip(self.attn_A,self.attn_v)):
                attn_output[:,:,i*self.head_dim:(i+1)*self.head_dim] = (torch.mul(A+1,attn_output[:,:,i*self.head_dim:(i+1)*self.head_dim].clone()) + v).clone()
        attn_output = self.o_proj(attn_output)
