...
from honest_llama.truthfulqa import utilities, models, metrics
from honest_llama.truthfulqa.configs import BEST_COL, ANSWER_COL, INCORRECT_COL
from honest_llama.truthfulqa.utilities import (
    format_prompt,
...
from honest_llama.truthfulqa.presets import preset_map, COMPARE_PRIMER
from honest_llama.truthfulqa.models import find_subsequence, set_columns, MC_calcs
from honest_llama.truthfulqa.evaluate import format_frame, data_to_dict
...
def train_probes(seed, train_set_idxs, val_set_idxs, separated_head_wise_activations, separated_labels, num_layers, num_heads):
    ...
    for layer in tqdm(range(num_layers)): 
        for head in range(num_heads): 
            ...
            # plot_heads[layer,head] = accuracy_score(y_val, y_val_pred)
            probes.append(clf)

    all_head_accs_np = np.array(all_head_accs)
    # import seaborn as sns
    # ax = sns.heatmap(np.sort(plot_heads,axis=-1)[:,::-1])
    # ax.set(ylabel='layer', xlabel='head')
    # ax.invert_yaxis()
    # ax.figure.savefig('prob acc')
    # error()
...
def get_interventions_dict(top_heads, probes, tuning_activations, num_heads, use_center_of_mass, use_random_dir, com_directions): 
    ...
    for layer, head in top_heads:
        # (random_layer,random_head) = random.sample(top_heads,1)[0]
        if use_center_of_mass: 
            direction = com_directions[layer_head_to_flattened_idx(layer, head, num_heads)]
            # direction = com_directions[layer_head_to_flattened_idx(layer, random_head, num_heads)]
        elif use_random_dir: 
            direction = np.random.normal(size=(128,))
        else: 
            direction = probes[layer_head_to_flattened_idx(layer, head, num_heads)].coef_
            # direction = probes[layer_head_to_flattened_idx(random_layer, random_head, num_heads)].coef_
